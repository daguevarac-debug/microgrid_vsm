...
4. bess_vs_no_bess: the 20% step with and without the DC-coupled BESS.

Every simulated model is constructed explicitly with ``GFMController`` at the
selected operating point ``(M, D) = (80, 1500)``. The no-BESS result from the
20% step is reused by the comparison, so the four reported scenarios require
four ODE integrations rather than five.

A scenario is marked ``FAIL`` only when execution, state integrity, controller
selection, or BESS physical-limit checks fail. A numerically valid scenario is
marked ``REVIEW`` when the existing frequency or DC-link design criteria are
not met. This distinction preserves the known severe-case evidence without
misclassifying a successful simulation as a software failure.
"""
...
STEP_20_BESS_SPEC = ScenarioSpec(
    name="load_step_20_with_bess",
    step_fraction=MICROGRID_LOAD_STEP_MODERATE_FRACTION_DEFAULT,
    with_bess=True,
)
...
def _reference_active_power_w() -> float:
    """Return the available active-power reference used by prior GFM studies."""
...
def build_model(spec: ScenarioSpec, p_ref_w: float) -> Microgrid:
    """Build one scenario with an explicitly active GFM controller."""
    ...
    model_type = MicrogridWithBESS if spec.with_bess else Microgrid
    model = model_type(load_profile=load_profile, controller=controller)

    if not isinstance(model.controller, GFMController):
        raise TypeError(
            f"{spec.name}: expected GFMController, got {type(model.controller).__name__}."
        )
    if model.controller_state_name != "omega":
        raise ValueError(
            f"{spec.name}: expected controller state 'omega', got "
...
def _initial_state(model: Microgrid) -> list[float]:
    if isinstance(model, MicrogridWithBESS):
        return model.initial_state_with_bess(vdc0=SIM_VDC0_V_DEFAULT)
...
def _bess_diagnostics(
    ...
    soh = np.asarray([model.bess.soh_from_z_deg(value) for value in z_deg], dtype=float)
    i_bess = np.asarray(
        [
            model._compute_i_bess(
                Vdc=float(vdc_value),
                soc_bess=float(soc_value),
                soh_bess=float(soh_value),
            )
            for vdc_value, soc_value, soh_value in zip(vdc, soc, soh)
        ],
        dtype=float,
    )
    p_bess_dc = vdc * i_bess
    i_limit = np.asarray(
        [model._available_i_bess_max(float(soh_value)) for soh_value in soh],
        dtype=float,
    )
    p_limit = np.asarray(
        [
            model._available_p_bess_max_w(float(soh_value))
    ...
    vt_bess = np.asarray(
        [
            model.bess.terminal_voltage(
                soc=float(soc_value),
                v_rc=float(vrc_value),
                i_bess=float(current_value),
                soh=float(soh_value),
            )
            for soc_value, vrc_value, current_value, soh_value in zip(
                soc, v_rc, i_bess, soh
            )
        ],
        dtype=float,
    ...
    signals_finite = bool(
        np.all(np.isfinite(i_bess))
        and np.all(np.isfinite(p_bess_dc))
        and np.all(np.isfinite(soc))
        and np.all(np.isfinite(soh))
        and np.all(np.isfinite(vt_bess))
    )
    soc_range_ok = bool(
        np.all((soc >= model.bess.soc_min) & (soc <= model.bess.soc_max))
    )
    soh_range_ok = bool(np.all((soh >= model.bess.soh_min) & (soh <= 1.0)))
    current_limit_ok = bool(np.all(np.abs(i_bess) <= i_limit + LIMIT_ATOL))
    power_limit_ok = bool(np.all(np.abs(p_bess_dc) <= p_limit + LIMIT_ATOL))
    ...
    return {
        **stress,
        ...
        "i_bess_limit_max_a": float(np.max(i_limit)),
        "p_bess_limit_max_w": float(np.max(p_limit)),
...
def run_scenario(
    ...
    *,
    ...
    record: dict[str, Any] = {
        ...
        "bess_active": bool(spec.with_bess),
    ...
    if spec.constant_load:
        ...
        if isinstance(model, MicrogridWithBESS):
            bess = _bess_diagnostics(model, solution)
            record["bess"] = bess
            if runtime_pass and not bool(bess["physical_limits_pass"]):
                record["status"] = "FAIL"
            else:
...
def _comparison_record(
    ...
    return {
        ...
        "same_load_profile": bool(
            no_bess["p_load_pre_step_w"] == with_bess["p_load_pre_step_w"]
            and no_bess["p_load_post_step_w"] == with_bess["p_load_post_step_w"]
        ),
...
def run_validation(
    *,
    ...
    step_20_bess, _, _ = run_scenario(
        STEP_20_BESS_SPEC,
        p_ref_w=p_ref_w,
        t_end_s=t_end_s,
    )
    comparison = _comparison_record(step_20, step_20_bess)
    ...
    report: dict[str, Any] = {
        ...
        "status_semantics": {
            ...
            "FAIL": "execution, state integrity, controller selection, or BESS limits fail",
        },
        "scenarios": scenarios,
    }
